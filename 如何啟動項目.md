...
**安裝後端依賴：**
1. 打開 PowerShell 或命令提示符
2. 導航到項目目錄：
   ```powershell
   cd "D:\School_IVE\IVE-HD IT114118\04_Sem04\ITP4221_IT_ICT_1( Artificial Intelligence Technologies ) by TUNG CHI CHEUNG\EA_project\hk-urban-butterfly-bird-id\web_app\backend"
   ```
3. 安裝 Python 依賴：
   ```powershell
...
**安裝前端依賴：**
1. 打開新的 PowerShell 或命令提示符窗口
2. 導航到前端目錄：
   ```powershell
   cd "D:\School_IVE\IVE-HD IT114118\04_Sem04\ITP4221_IT_ICT_1( Artificial Intelligence Technologies ) by TUNG CHI CHEUNG\EA_project\hk-urban-butterfly-bird-id\web_app\frontend"
   ```
3. 安裝 Node.js 依賴：
   ```powershell
   npm install
   ```
   ⚠️ **注意**：首次安裝可能需要 2-5 分鐘
...
- [ ] 前端依賴已安裝（`npm install`）
- [ ] 模型文件存在（`models\trained\model.h5`）
- [ ] 類別文件存在（`models\trained\class_names.json`）
- [ ] 兩個服務都已啟動
